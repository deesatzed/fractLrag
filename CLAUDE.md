...
├── storage.py              # SQLite persistence (schema v2, doc_metadata table)
└── benchmarks/
...
| `retrieve_adaptive()` | **Recommended** | Auto-classifies query type, routes spec→flat, others→reranked |
...
### Cross-Validated (5-fold stratified CV, 2026-04-22)
...
| p-value | — | 0.060 | borderline |
| 95% CI | — | [+0.004, +0.066] | excludes zero |

Per-type: summary +8.0%, spec +1.7%, synthesis +1.9%, logic +0.8%
...
The +7.3% non-CV number is inflated due to tuning on the evaluation set. The honest CV number is +4.1%.
